...
#for x in yaml("content.yml") {
  let content-list = x.at(1)
  for i in range(0, content-list.len()) {
    let folder-name = content-list.keys().at(i)
    let title-section = title-case(folder-name.split("-").join(" "))
    template-section-title(title-section)

    for file in content-list.at(folder-name) {
      let file-name = if type(file) == dictionary {
        file.keys().at(0)
      } else {
        file
      }
      let template-title = if type(file) == dictionary {
        file.values().at(0)
      } else {
        title-case(file-name.split(".").at(0).split("-").join(" "))
      }
